...
export class Starlight extends Drawable {
  ...
  draw(): void {
    ...
    for (let i = 0; i < this.numStars; i++) {
      ...
      const t = p.map(this.starBrightness[i], 100, 255, 0, 1); // Interpolation factor based on brightness
      const starColor = p.lerpColor(p.color(0), p.color(255, 255, 255), t); // Gradient from yellow to white
